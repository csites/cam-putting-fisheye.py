...
The last addition to the code allows you to change the webcam number with the 'c' key.  It simply increments the webcam number cycling through them 0-3.  You can only have a nmaximum of 4 USB webcams per USB controller and I make the assumption that you have only one USB controller so it only cycles through the first 4. I added this feature because, the webcam number would often change making the makeing the webcamindex in the config.ini [camera parameters] wrong, so to avoid having to leave and edit that file, this addition changes the webcam number.  If you perforrm a 'w' to write the config.ini it the new value will be saved.  Note:  Not only are you changing the camera index, but it also changes the webcams [camera parameters] and sets them to the defaults for that webcam index.  
</p> 
